...
# Get test mode from first argument
TEST_MODE="${1:-all}"

echo "🐳 Запуск тестов в Docker контейнере (режим: $TEST_MODE)"
echo "================================================================"
...
log_info "Запуск тестов в контейнере (режим: $TEST_MODE)..."
...
case "$TEST_MODE" in
    "api"|"api-local"|"ui"|"smoke"|"fast"|"all"|"help")
        log_info "Выбран режим: $TEST_MODE"
        ;;
    *)
        log_error "Неизвестный режим: $TEST_MODE"
        echo "Доступные режимы: api, api-local, ui, smoke, fast, all, help"
        exit 1
        ;;
esac
...
echo "✅ Готово к CI/CD"
echo "✅ Параметризованный запуск тестов"
echo ""
echo "🔍 Примеры запуска:"
echo "  ./scripts/run_in_docker.sh api       # Только API тесты"
echo "  ./scripts/run_in_docker.sh ui        # Только UI тесты"
echo "  ./scripts/run_in_docker.sh fast      # Быстрые тесты"
echo "  ./scripts/run_in_docker.sh all       # Полный комплекс тестов"
